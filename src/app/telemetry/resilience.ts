...
export function chunkArray<T>(arr: readonly T[], size: number): T[][] {
  ...
  // Perf note: pre-size the output array so V8 allocates the backing store once
  // instead of resizing it on each push(). For the common case of 500 events /
  // chunk-size 200 this allocates 3 slots upfront — zero dynamic resizing.
  const numChunks = Math.ceil(arr.length / size);
  const chunks: T[][] = new Array(numChunks);
  for (let i = 0; i < numChunks; i++) {
    chunks[i] = arr.slice(i * size, (i + 1) * size);
  }
  return chunks;
}
